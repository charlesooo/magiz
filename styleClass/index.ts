          ...
          try {
            ...
            if (styles.data.preset) {
              ...
              if (name) {
                ...
                // 按关键词随机选择预设样式
                const names = Object.keys(styles.data.preset).filter((n) =>
                  n.includes(key)
                );
                if (names.length > 0) {
                  presetData =
                    styles.data.preset[sample(names, seed) as string];
                }
              }
              ...
              if (presetData) {
                  ...
                  if (color) {
                    for (const key in presetData.color) {
                      GLOBAL.COLOR_PRESET[key] =
                        color[key] ||
                        (presetData.color[key] as string | string[]);
                    }
                  } else {
                    for (const key in presetData.color) {
                      GLOBAL.COLOR_PRESET[key] = presetData.color[key] as
                        | string
                        | string[];
                    }
                  }
                ...
                presetData.floor.forEach((f) => {
                  ...
                  if (floorParams.floorControl)
                    fp.floorControl = floorParams.floorControl;
                  if (floorParams.floorNumber)
                    fp.floorNumber = floorParams.floorNumber;
                  if (floorParams.floorRange)
                    fp.floorRange = floorParams.floorRange;

                  // 边线控制可以叠加
                  fp.setEdges = [
                    ...(floorParams.setEdges || []),
                    ...(fp.setEdges || []),
                  ];
...
function parseFloor(
  ...
  ranges.forEach((range) => {
    ...
    // 修改层数后 SH 也变了
    GLOBAL.UNITS.SH =
      floorNumber || floorRange ? total * floorHeight : sectionHeight;
...
function parseEdgeParams(
  params: styleTypes.floor
): styleParsed.handleEdgesType {
  return {
...
function parseControl(
  params?: styleTypes.control
): styleParsed.control | undefined {
  return params
    ? {
            ...
            parsed.proto.push({
              ...
              spacing: spacing?.map((s) => {
                return {
                  ...
                  group: s.group
                    ? parseBoxGroup(
                        Array.isArray(s.group) ? s.group : [s.group]
                      )
                    : undefined,
              ...
              divide: divide?.map((d) => {
                return {
                  count: parse(d.count),
                  group: parseBoxGroup(
                    Array.isArray(d.group) ? d.group : [d.group]
                  ),
...
function parseBoxInside(
  ...
  if (boxInside) {
    boxInside.forEach((boxParams) => {
      const { flex, count, widthRatio, depthRatio, heightRatio, along, once } =
        boxParams;
...
function parseClamp(
  params?: styleTypes.clampRangeType
): styleParsed.clampRangeType | undefined {
  if (params) {
    return {
