    ...
    if (floorCount > 0) {
      section.floor?.forEach((floorParams) => {
        ...
        floorParams.presets?.forEach((floorPresetParams) => {
          // 将预设的单位换算缓存到全局变量中
          const { unit, color } = floorPresetParams
          GLOBAL.UNITS_PRESET = {}
          for (const key in unit) {
            GLOBAL.UNITS_PRESET[key] = parse(unit[key as keyof typeof unit])
          }

          // 将预设的颜色值缓存到全局变量中
          GLOBAL.COLOR_PRESET = {}
          for (const key in color) {
            GLOBAL.COLOR_PRESET[key] = color[key as keyof typeof color]!
          }

          floorPresetParams.floor.forEach((presetFloorParams) => {
            // 创建预设的深拷贝
...
function parseFloor(
  ...
  const control = parseControl(floorParams.control)
  ...
  // 获取层数范围
  indexesPassedControl(totalFloors, control).forEach((i, n) => {
    const isFirst = n === 0
    const floorElevation = sectionElevation + i * floorHeight
    parseExtrude(floorElevation, isFirst, saveAs, floorParams.extrude)
    parseSlopingRoof(floorElevation, isFirst, saveAs, floorParams.slopingRoof)
    parseClampBox(floorElevation, isFirst, saveAs, floorParams.clampBox)
    parseMatch(floorElevation, isFirst, saveAs, floorParams.match)
    parseBoxInside(floorElevation, isFirst, saveAs, floorParams.boxInside)
    parseAdjuncts(floorElevation, isFirst, saveAs, floorParams.adjunct)
    parseFacade(floorElevation, isFirst, saveAs, floorParams.facade)
  })
}
...
function parseControl(
  ...
  return params
    ? {
        ...
        last: parse(params.last),
        first: parse(params.first),
