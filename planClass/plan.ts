...
export default class PLAN {
  ...
  constructor(input: magizTypes.requestData) {
    ...
    this.orignal = input.loops.map((loop) =>
      loop.map((pt) => new Vector2(...pt))
    );
    ...
    if (outterLoop && outterLoop.length > 2) {
      ...
      outterLoop.forEach((current, i) => {
        const next = outterLoop[
          i + 1 === outterLoop.length ? 0 : i + 1
        ] as Vector2;
      ...
      if (p.offset) {
        ...
        switch (o) {
          case "WIDTH":
            rays = rays
              .map((loop) =>
                loop.filter((ray) => isAlongAxis(ray.direction.angle()))
              )
          ...
          case "LONGEST":
            {
              ...
              rays =
                o === "LONGEST"
                  ? rays.map((loop) =>
                      loop.filter((ray) =>
                        isAlongAxis(ray.direction.angleTo(r.direction))
                      )
                    )
                  : rays.map((loop) =>
                      loop.filter(
          ...
          default:
            rays = rays
              .map((loop) =>
                loop.filter(
                  (ray) =>
                    Math.abs(ray.direction.angle() - o) < 10 * (Math.PI / 180)
                )
              )
              .filter((loop) => loop.length > 0);
            break;
  ...
  toModel(
    ...
    const styleParsed = styles.parseStyle(
      this.styleParams,
      this.seed,
      customStyles
    );
    ...
    const result: magizTypes.rawBuilding = {
      ...
      points: this.relative.rays.map((loop) =>
        loop.map((ray) => ray.start.toArray())
      ),
      center: this.center.toArray(),
      centerRelative: [
        this.center.x - centerOfAll.x,
        this.center.y - centerOfAll.y,
      ],
...
function getClampedRects(
  ...
  const w = max.x - min.x;
  const h = max.y - min.y;
  const { xMin, xMax, yMin, yMax, xCentral, yCentral, asRatio, reverse } =
    params;
