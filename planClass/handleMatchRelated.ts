  ...
  parsed.forEach((boxInside) => {
    ...
    if (matchData.length > 0) {
      ...
      for (let i = 0; i < total; i++) {
        ...
        const pi = Math.floor(matchData[matchDataIndex].pairs.length * rand(seed))
        ...
        selected.push({ matchDataIndex, props })
      }
      ...
      selected.forEach((c, i) => {
        ...
        if (props.d > 0) {
          ...
          matrix = applyTransform(data, matrix)
            .premultiply(TEMP.makeTranslation(props.x + moveX, props.y + moveY, elevation + h / 2))
            .premultiply(TEMP.makeRotationZ(-radian))

          const color = sample(data.color, seed) || DEFAULT_COLOR
          const saveAs: instancedDataType = result.boxData['box' + (color.glass ? 'Glass' : '')]
  ...
  parsed.forEach((adjunct) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      boxes.forEach((box) => {
        ...
        applyTransform(box, matrix).premultiply(TEMP.makeTranslation(point.x, point.y, elevation))
        const saveAs: instancedDataType = result.boxData['box' + (color.glass ? 'Glass' : '')]
        saveAs.matrices.push(matrix.toArray())
        saveAs.colors.push(color.index)
      })
...
/** 返回多边形的随机内部点 */
function randomPointInPolygon(loops: temp.line[][], seed: SEED): Vector2 {
  // 多边形沿Y轴的范围内随机取点
  ...
  return new Vector2(pair[0].x + (pair[1].x - pair[0].x) * rand(seed), randomY)
}

/** 返回多边形边线上的随机点 */
function randomPointOnEdge(loops: temp.line[][], seed: SEED): Vector2 {
  const { start, end } = sample(loops[0], seed)
  const t = rand(seed)
...
function collectPairs(
  ...
  const { center, width } = matchData[matchIndex].pairs[pairIndex]
  let xLeftMin = center.x - width / 2
  let xRightMax = center.x + width / 2
  let centerMinY = center.y
  let centerMaxY = center.y

  matching(matchIndex, 1)
  matching(matchIndex, -1)

  return {
    w: xRightMax - xLeftMin,
    d: centerMaxY - centerMinY,
    x: (xLeftMin + xRightMax) / 2,
    y: (centerMinY + centerMaxY) / 2,
  }

  /** 从index开始朝一个方向进行拟合，同步更新 xLeftMin, xRightMax, yMin, yMax */
  function matching(index: number, direction: number) {
    let found = true
    while (count > 0 && found) {
      found = false
      index += direction
      matchData[index]?.pairs.forEach((pair) => {
        ...
        if (!(xR <= xLeftMin || xL >= xRightMax)) {
          if (xLeftMin < xL) xLeftMin = xL
          if (xRightMax > xR) xRightMax = xR
          if (direction > 0) {
            centerMaxY = center.y
          } else {
            centerMinY = center.y
